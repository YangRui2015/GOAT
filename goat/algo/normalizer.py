...
from goat.algo.util import reshape_for_broadcasting
...
class Normalizer:
    def __init__(self, size, eps=1e-2, default_clip_range=np.inf, sess=None):
        ...
        self.mean_numpy = np.zeros(self.size, np.float32)
        self.std_numpy = np.ones(self.size, np.float32)
    ...
    def update(self, v):
        ...
        with self.lock:
            ...
            self.mean_numpy = self.local_sum / self.local_count
            self.std_numpy = np.maximum(self.local_sumsq / self.local_count - np.square(self.local_sum / self.local_count), 
                                        np.square(self.eps))
