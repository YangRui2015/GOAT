...
class WGCSL(object):
    ...
    def __init__(self, input_dims, buffer_size, hidden, layers, network_class, polyak, batch_size,
                ...
                use_vex=False, obs_to_goal=None, use_kde=False, weight_ratio=1, weight_min=0.5, weight_max=1.0,
                use_iql=True, iql_tau=0.7,
                **kwargs):
        ...
        # smooth 
        self.policy_smooth_reg = psmooth_reg   #1
        self.Q_smooth_reg = qsmooth_reg        # 0.1
        ...
        if self.use_supervised:
            ...
            info = {
                ...
                'ensemble': self.ensemble_actions,
                'get_Q': self.get_Q,
        ...
        self.buffer = ReplayBuffer(buffer_shapes, buffer_size, self.T, sampler, self.sample_transitions, info, validation_rate=0)  #0
    ...
    def ensemble_actions(self, o, g):
        o = np.clip(o, -self.clip_obs, self.clip_obs)
        g = np.clip(g, -self.clip_obs, self.clip_obs)

        policy = self.main ##self.target if use_target_net else
        out = self.sess.run([policy.pi_tf_std], feed_dict={  # policy.pi_tf, policy.pi_tf_list, 
            policy.o_tf: o.reshape(-1, self.dimo),
            policy.g_tf: g.reshape(-1, self.dimg)
        })
        return out
    ...
    def get_Q_pi(self, o, g, std=False):
        ...
        if not std:
            ...
            out = [policy.Q_pi_tf, policy.Q_pi_tf_std] #  policy.Q_pi_tf_list[0]
            ret = self.sess.run(out, feed_dict=feed)
        return ret
    ...
    def train_policy(self, o, g, u, weights=None, update=True, original_g=None):
        ...
        #     pi_sl_loss, pi_sl_grad, policy_smooth_loss = self.sess.run([self.policy_sl_loss, self.pi_sl_grad_tf, self.policy_smooth_loss], feed_dict=feed_dict)
        #     print(policy_smooth_loss)
    ...
    def _create_network(self, reuse=False):
        ...
        for target_Q_pi_tf, main_Q_tf in zip(target_Q_pi_tf_list, main_Q_tf_list):
            ...
            if self.use_iql:
                ...
                iql_weight = tf.ones(self.batch_size) * self.iql_tau + (1 - 2 * self.iql_tau) * pos_idxs_float
                Q_loss_tf = (self.batch_size / tf.reduce_sum(iql_weight)) * tf.reduce_mean(iql_weight * tf.square(Q_diff)) 
            else:
        ...
        # weight_loss_lis = []
        # for main_pi_tf in self.main.pi_tf_list:
        #     weight_loss_temp = tf.reduce_mean(tf.square(self.main.u_tf - main_pi_tf),axis=1)
        #     weight_loss_lis.append(tf.reduce_mean(self.gcsl_weight_tf * weight_loss_temp) )
            
        # self.policy_sl_loss = tf.reduce_sum(weight_loss_lis) 
        # self.policy_sl_loss +=  1 * tf.reduce_mean(self.main.pi_tf_std)
