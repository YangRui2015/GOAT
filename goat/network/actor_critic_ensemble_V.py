...
class ActorCritic_Ensemble: 
    ...
    def __init__(self, inputs_tf, dimo, dimg, dimu, max_u, o_stats, g_stats, hidden, layers, 
                  ensemble_size=5,  use_bilinear=False, **kwargs):
        ...
        with tf.variable_scope('V'):
            ...
            self.V_tf_list = create_nn(input_V, V_layers, ensemble_size=self.ensemble_size, name=self.net_type+'/V')
            self.V_tf = tf.reduce_mean(self.V_tf_list, axis=0)
            self.V_tf_std = tf.math.reduce_std(self.V_tf_list, axis=0)
